...
cron.property('cronExpression', {
  'type': 'string',
  'default': '*/60 * * * *', // defaults to once every 60 minutes
  'required': true,
  'minLength': 1,
  'maxLength': 50
});
...
cron.getStatus = function getStatus (cb) {
  ...
  cron.getKey('/lastCronBatch', function (err, lastCronBatch) {
    cb(err, {
      lastCronBatch: lastCronBatch
    });
  });
};
...
function onlyUniqueCronNames (data, next) {
  ...
  return cron.find({
    ...
    if (results.length > 0 && results[0].id !== data.id) {
      var msg = 'Cron already exists ' + '/' + data.owner + "/" + data.name;
...
function limitMinimumCronInterval (data, next) {
  if (typeof data.cronExpression === 'undefined') {
    return next(null, data);
  }
  var limit = servicePlan[data.servicePlan].cronMinimalInterval;
  var expression = data.cronExpression;
  try {
    var interval = parser.parseExpression(expression);
    // calculate the next date
    var nextTime = interval.next()._date.format('x');
    // calculate the previous date
    var prevTime = interval.prev()._date.format('x');
    var diff = nextTime - prevTime;
    if (diff < limit) {
      return next(new Error('Interval is too frequent per service plan ' + (diff / 1000) + " is not enough seconds"))
    }
    return next(null, data);
  } catch (err) {
    return next(new Error('Invalid cron expression: ' + data.cronExpression));
  }
}

cron.before('update', limitMinimumCronInterval);
cron.before('create', limitMinimumCronInterval);

cron.before('update', onlyUniqueCronNames);
cron.before('create', onlyUniqueCronNames);
...
function setNextExecution (data, next) {
  try {
    ...
    next(new Error('Invalid cron expression: ' + data.cronExpression));
  }
}
