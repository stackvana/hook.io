...
module['exports'] = function forkHook (req, res) {
    ...
    if (!req.isAuthenticated()) {
      req.session.redirectTo = "/" + req.params.username + "/" +  req.params.hook + "?fork=true";
    ...
    return hook.find(query, function(err, result){
      ...
      if (req.user.username === username) {
        return res.end('You cannot fork your own Hooks. Try creating a new Hook instead?');
      }
      ...
      hook.find({ owner: req.user.username, name: h.name }, function(err, result){
        ...
        if(result.length > 0) {
          return res.end("You've already forked this Hook at " + "http://hook.io/" + req.user.username + "/" + h.name);
        }
        forkScript();
      })
