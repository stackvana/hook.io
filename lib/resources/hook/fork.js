  ...
  var github = new GitHubApi({
    ...
    return hook.find(query, function(err, result){
      ...
      if (req.session.user === h.owner) {
        return res.end('You are already the owner of this service, so you cannot fork it.\nShare this link with other developers so they may fork it.');
      }
      ...
      hook.find({ owner: req.session.user, name: h.name }, function (err, result) {
        ...
        if(result.length > 0) {
          var r = {
            error: true,
            message: "You've already forked this Hook at " + config.app.url + "/" + req.session.user + "/" + h.name
          };
          if (req.jsonResponse) {
            return res.json(r);
          } else {
      ...
      */
      function forkScript (h) {
        var _hook = {
          ...
          language: h.language,
          mschema: h.mschema,
          mschemaStatus: h.mschemaStatus,
          themeSource: h.themeSource,
          presenterSource: h.presenterSource,
          themeStatus: h.themeStatus,
          schemaStatus: h.schemaStatus
        };
