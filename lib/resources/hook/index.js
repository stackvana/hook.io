...
hook.after('update', updateCronProvider);
hook.after('create', updateCronProvider);

function updateCronProvider (data, next) {
  var key = '/' + data.owner + '/' + data.name;
  if (data.cronActive) {
    cron.sadd('crons', key, function (err) {
      cron.set(key, {
        owner: data.owner,
        name: data.name,
        cron: data.cron,
        uri: key,
        lastRan: data.lastRan || null,
        nextRun: new Date()
      }, function (err) {
        next(err, data);
      });
    ...
    cron.srem('crons', key, function (err) {
      cron.del(key, function (err){
        next(err, data);
      });
    });
  }
