...
module['exports'] = function preprocessHook (opts, userModule, callback) {
  ...
  // process incoming route parameters if hook.path exists
  if (typeof req.hook.path !== "undefined" && req.hook.path.length > 0 
     && typeof req.params["0"] !== "undefined" && req.params["0"].length > 0) {
    var parts = req.hook.path.split('/');
    var incomingParams = req.params["0"].split('/');
    parts.forEach(function(part, i){
      if (part.substr(0,1) === ":") {
        req.resource.params[part.substr(1, part.length -1)] = incomingParams[i-1];
      }
    });
  }
