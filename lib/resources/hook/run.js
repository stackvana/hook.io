      ...
      hook.determineRequestFormat(req , res, function (req, response) {
        ...
        hook.runHook({ req: req, res: res }, function (err, response) {
          ...
          if (err) {
            ...
            // we are still doing schema validation in `worker` itself before being sent to spawned process
            // this means if validation error or other before spawn, decrement the count
