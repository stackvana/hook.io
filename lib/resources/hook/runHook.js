      ...
      */
      ...
      function _runHook () {
        ...
        resource.emit('hook::run', {
          ...
          // ip: req.connection.remoteAddress, // why was this removed? we do need it. if anything, filter out this property in /:username/events logs for public services
          url: req.url
        });
          ...
          function _finishSpawn () {
            ...
            */
            ...
            function _spawnService (_req, _res) {
              if (config.worker.useNSJAIL) {
                untrustedService.jail = "nsjail";
                untrustedService.jailArgs = config.worker.nsJailArgs;
              ...
              untrustedService.endResponseOnExit = false;
