          ...
          function _finishSpawn () {
            ...
            */
            ...
            if (untrustedService.themeStatus !== "disabled" && untrustedService.view) {
              // console.log("CALLING VIEW PRE".yellow)
              stack.viewPresenter(untrustedService, req, res, function (err, req, output){
                if (err) {
                  return callback(err);
                }
                // console.log("CALLED THE VIEW POST".green)
            ...
            function _spawnService (req, res) {
              ...
              if (config.useNSJAIL) {
                ...
                untrustedService.jailArgs = [ '-Mo', '--chroot', '/var/chroot/', '--user', '99999', '--group', '99999', '--rlimit_as', '9999', '--rlimit_nproc', '1000' ,'--quiet', '--'];
              }
              ...
              stack.plugins.spawn(untrustedService)(req, res, function _serviceEnded (err, result) {
                ...
                //
                // if the callback made it this far, it's possible we have additional middlewares to process,
