    ...
    switch (targetLanguage) {
        ...
        for (var p in __env) {
          if (typeof __env[p] === "object") {
            for (var s in __env[p]) {
              bashInject += 'Hook_' + p + '_' + s + '="' + bashEscape(__env[p][s]) + '";\n'
            }
          } else {
            bashInject += 'Hook_' + p + '="' + bashEscape(__env[p]) + '";\n'
          }
        }
        binaryArgs = [
      ...
      case 'lua':
        ...
        for (var p in __env) {
          if (typeof __env[p] === "object") {
            for (var s in __env[p]) {
              luaInject += 'Hook_' + p + '_' + s + ' = \'' + luaEscape(__env[p][s]) + '\'\n'
            }
          } else {
            luaInject += 'Hook_' + p + ' = \'' + luaEscape(__env[p]) + '\'\n'
          }
        }
      ...
      case 'perl':
        ...
        for (var p in __env) {
          if (typeof __env[p] === "object") {
            for (var s in __env[p]) {
              perlInject += '$Hook_' + p + '_' + s + ' = \'' + perlEscape(__env[p][s]) + '\';\n'
            }
          } else {
            perlInject += '$Hook_' + p + ' = \'' + perlEscape(__env[p]) + '\';\n'
          }
        }
        binaryArgs = [
          '-code', service.evalSource,
      ...
      case 'smalltalk':
        ...
        for (var p in __env) {
          if (typeof __env[p] === "object") {
            ...
            smalltalkInject += "Hook_" + p + " := \'" + smalltalkEscape(__env[p]) + "\'.\n";
          }
        }
        ...
        for (var p in __env) {
          if (typeof __env[p] === "object") {
            for (var s in __env[p]) {
              ...
              tclInject += "set Hook_" + p + " \"" + tclEscape(__env[p]) + "\"\n";
          }
        }
        binaryArgs = [
