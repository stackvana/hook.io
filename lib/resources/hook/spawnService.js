...
module['exports'] = function spawnService (service, input, output) {
 ...
 var debugOutput = [];
  ...
  vm.stderr.on('data', function (data) {
    ...
    stderr.onData(data, status, output);
  });
  ...
  vm.on('error', function (err) {
    ...
    // ended = true;
    if (!status.ended) {
      // should we write output here?
      //output.write(err.message)
    }
  });

  vm.on('close', function (code) {
    // console.log(' vm closed? ' + code, status.checkingRegistry, status.ended);
    status.vmClosed = true;
    if(!status.checkingRegistry && !status.ended && !status.stdoutEnded) {
      status.ended = true;
      // console.log('vm close called output.end()');
      output.end();
    }
  });
