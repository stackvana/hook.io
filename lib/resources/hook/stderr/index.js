...
stderr.onData = function onStderrData (data, status, output) {
  ...
  if (messages.substr(0, 1) !== "{" && messages.substr(0, 14) !== "\nmodule.js:333") {
    ...
    return handleMessage(message, status, output);
  } 
  ...
  messages.forEach(function(message){
    ...
    handleMessage(message, status, output);
  });
};

var handleMessage = stderr.handleMessage = function (message, status, output) {
  ...
  */
  ...
  if(typeof responseMethods[message.type] === "function") {
    responseMethods[message.type](output, message);
    return;
  }

  // if the incoming message is end event, signal that is time to end the response
  if (message.type === "end") {
    if (!status.ended && !status.checkingRegistry && !status.erroring) {
      status.ended = true;
      // output.end();
    }
  }
  ...
  if (message.type === "error") {
    ...
    if (message.payload.code === "MODULE_NOT_FOUND") {
      ...
      // console.log('writing error', message.payload.error, status);
