...
keys.after('destroy', function (data, next){
  var keyPath = '/keys/' + data.owner + '/' + data.hook_private_key;
...
keys.before('create', function (data, next){
  ...
  user.find({ name: data.owner }, function (err, res) {
    ...
    keys.find(query, function (err, res) {
      ...
      // determine current users service plan limits 
      // note: do not use session scope since checkRoleAccess() might not have session ( from public API usage )
      var plan = u.servicePlan || 'trial';

      // free plans are now trial plans
      if (plan === 'free') {
        plan = 'trial';
      }

      var serviceLimits = servicePlan[plan];

      // Use session.serviceLimits to only allow n keys per user account based on current subscription plan
      if (res.length > serviceLimits.apiKeys) {
        var keyNoun = "key";
        if (serviceLimits.apiKeys > 1) {
          keyNoun = "keys";
        }
        var msg = 'You may not create more than ' + serviceLimits.apiKeys + ' access ' + keyNoun + ' with current account plan: ' + plan + '\n\nPlease see ' + config.app.url + '/pricing for more details.';
        return next(new Error(msg), data);
      }
      // if all checks pass, continue forward and create key
      next(null, data);
    });
  })
