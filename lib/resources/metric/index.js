...
metric.all = function (owner, service, cb) {
  var results = {};
  metric.zscore('running', owner, function(err, score){
    results['/' + owner + "/running"] = score;
    metric.zscore('hits', owner, function(err, score){
      results['/' + owner + '/hits'] = score;
      metric.zscore('hits', owner + "/" + service, function(err, score){
        results['/' +  owner + "/" + service + "/hits"] = score;
        cb(err, results);
      });
    });
  });
...
metric.top = function (zset, cb) {
  var args = [ 'metrics/' + zset, '+inf', '-inf', 'WITHSCORES', 'LIMIT', 0, 50 ];
