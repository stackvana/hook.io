...
modules.checkRegistry = function checkRegistry (package, cb) {
  ...
  if (start === "." || start === "/" || start === "\\") {
    ...
    str +=    "Any additional modules outside of the core Node.js api must be required through npm.";
  ...
  npm.view(package, function(err, res){
    ...
    if (err) {
      return cb(new Error('Error communicating with npmjs.org ' + err.message));
    }
    return cb(null);
  });
