...
user.property('lastAlerted', 'string');
user.property('servicePlanMeta', 'object');
...
user.method('login', function (opts, cb) {
  ...
  function _login () {
    ...
    console.log("who is thi suser", u);

    user.setCache(u, function(){
...
user.setCache = function userSetCache (data, next) {
  var userKey = '/user/' + data.name;
  cache.set(userKey, data, function (err, re) {
    next(err, data);
  });
};

// returns the cached version if exists, or will set couch version into cache for next time
user.getSetCache = function (opts, finish) {
  var key = '/user/' + opts.name;
  // TODO: move cache requests inside of resource.before('get')
  cache.get(key, function (err, _user) {
   if (_user === null) {
     getUserFromCouch(function (err, u) {
       if (err) {
         return finish(err);
       }
       cache.set(key, u, function () {
         postProcessUser(err, u);
       });
     });
   } else {
     postProcessUser(null, _user);
   }
  });
  function getUserFromCouch (cb) {
    user.findOne({ name: opts.name }, function (err, result) {
      if (err) {
        return cb(err);
      }
  ...
  function postProcessUser (err, re) {
    // Perform join of current service plan limits onto user
    // console.log(servicePlan, user.servicePlan);
    re.servicePlanMeta = servicePlan[re.servicePlan];
    finish(err, re);
  }
};

user.after('create', function(data, next){
  user.setCache(data, next);
});

user.after('update', function(data, next){
  user.setCache(data, next);
});
