...
server.start = function start (opts, cb) {
  ...
  http.listen({
    ...
    // TODO: refactor application of handlers that take in :owner / :hook / :subhook
    // All share similiar logic, can reduce lines of code
    app.get('/:owner/:hook/refresh', function (req, res){
      return handleRefresh(req, res);
    });

    app.get('/:owner/:hook/:subhook/refresh', function (req, res){
      return handleRefresh(req, res);
    });
    ...
    app.post('/:owner/:hook/:subhook/refresh', function (req, res){
      return handleRefresh(req, res);
    });
    ...
    app.get('/:owner/:hook/:subhook/fork', function (req, res){
    ...
    app.post('/:owner/:hook/:subhook/fork', function (req, res){
      req.params.hook = req.params.hook + "/" + req.params.subhook;
      return res.redirect(config.baseUrl + "/" +  req.params.owner + '/' + req.params.hook + '?fork=true');
    });

    app.get('/:owner/:hook/:subhook/admin', function (req, res){
      return res.redirect(config.baseUrl + '/admin?owner=' + req.params.owner + '&name=' + req.params.hook + "/" + req.params.subhook);
    });
