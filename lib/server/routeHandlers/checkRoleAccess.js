    ...
    if (privateKey !== null && privateKey.length > 0) {
      ...
      cache.get(keyPath, function (err, _key) {
        ...
        if (_key === null) {
          ...
          //console.log('CACHED KEY'.yellow, opts.role, _key);
          finish(null, _key);
        }
      });
      function finish (err, result) {
        ...
        if (owner === "anonymous") {
          // if the owner is anonymous ( such as the /env or /datastore endpoints ),
          // and we have made it here, assume that key was matched and is valid
          owner = result.owner;
          hasAccess = true;
        }
