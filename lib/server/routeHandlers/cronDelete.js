...
module['exports'] = function handleCronDelete (req, res) {
  ...
  function deleteCron () {
    ...
    return cron.find({ owner: req.resource.owner, name: req.params.cron }, function (err, result){
      if (err) {
      ...
      return h.destroy(function(err){
        ...
        cache.del('/cron/' + req.params.owner + '/' + req.params.cron, function (){
          ...
          if (req.jsonResponse === true) {
            var msg = {
              status: 'deleted',
              message: 'Cron "' + req.params.cron + '" has been deleted!',
              owner: req.params.owner,
              name: req.params.cron
            };
            return res.json(msg);
          } else {
            return res.redirect(config.app.url + '/cron');
          }
        });
      });
    });
