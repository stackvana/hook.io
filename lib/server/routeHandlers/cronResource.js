...
module['exports'] = function handleCronResource (req, res) {
  ...
  var key = '/cron/' + req.params.owner + '/' + req.params.cron;
  ...
  function findCron(cb) {
    cron.find({ owner: req.params.owner, name: req.params.cron }, function (err, result) {
      if (err) {
        return res.end(err.message);
      }
      if (result.length === 0) {
        res.statusCode = 404;
        return res.end('Not found: ' + req.params.owner + '/' + req.params.cron);
      }
      req.cron = result[0];
      cb(null, result[0]);
    });
