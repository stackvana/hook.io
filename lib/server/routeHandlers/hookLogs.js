    ...
    return hook.find(query, function(err, result){
      ...
      if (result.length === 0) {
        ...
        if (req.session.user !== 'anonymous') {
          req.params.owner = req.session.user;
        }
      ...
      if (params.flush) {
        return checkRoleAccess({ req: req, res: res, role: "hook::logs::write" }, function (err, hasPermission) {
          ...
          // TODO: add better response codes from flush 1, 0, null, error, etc
      ...
      checkRoleAccess({ req: req, res: res, role: "hook::logs::read" }, function (err, hasPermission) {
        ...
        if (req.resource.owner === 'anonymous') {
        ...
        if (req._readableState && req._readableState.buffer && req._readableState.flowing) {
          isStreaming = true;
        }

        if (params.streaming === "true" || params.s === "true") {
          isStreaming = true;
        }
