...
if (process.platform === "darwin") {
  ...
  /*
  config.web.redis.host = "0.0.0.0";
  config.balancer.redis.host = "0.0.0.0";
  config.broadcast.redis.host = "0.0.0.0";
  config.redis.host = "0.0.0.0";
  config.redisCache.host = "0.0.0.0";
...
server.start = function start (opts, cb) {
      ...
      http.listen(defaultConfig, function (err, app) {
        ...
        */
        ...
        app.use(function (req, res, next) {
          console.log(new Date(), app.server.address().address, app.server.address().port, req.method, req.url, req.params);
          /*
          var opt = {
            request: req,
        ...
        var handleCron = require('../server/routeHandlers/cron');

        app.all('/cron/:owner/:cron', function (req, res){
          req.params.owner = req.params.owner.toLowerCase();
          req.params.cron = req.params.cron.toLowerCase();
          handleCron(req, res, app)
        });

        var handleDestroyCron = require('../server/routeHandlers/cronDelete');
        app.all('/cron/:owner/:cron/destroy', function (req, res){
          req.params.owner = req.params.owner.toLowerCase();
          req.params.cron = req.params.cron.toLowerCase();
          handleDestroyCron(req, res, app)
        });

        app.all('/cron/:owner/:cron/admin', function (req, res){
          req.params.owner = req.params.owner.toLowerCase();
          req.params.cron = req.params.cron.toLowerCase();
          return res.redirect(config.app.url + '/cron/admin?owner=' + req.params.owner + '&name=' + req.params.cron + '');
        });

        var handleCronResource = require('../server/routeHandlers/cronResource');
        app.all('/cron/:owner/:cron/resource', function (req, res){
          req.params.owner = req.params.owner.toLowerCase();
          req.params.cron = req.params.cron.toLowerCase();
          handleCronResource(req, res, app);
        });
        ...
        */
        ...
        app.get('/gateway/logs', function (req, res){
          req.params.owner = 'anonymous';
          req.params.hook = 'gateway';
          handleLogs(req, res);
        });
        ...
        app.get('/metrics/:owner/report', function (req, res){
          req.params.owner = req.params.owner.toLowerCase();
          metric.hgetall('/' + req.params.owner + '/report', function (err, report) {
            if (err) {
              return res.end(err.message);
            }
            if (report === null) {
              return res.json({ error: true, message: 'metric not found'})
            }
            res.json(report);
          });
        });
        ...
        app.get('/metrics/:owner/:hook/report', function (req, res){
          req.params.owner = req.params.owner.toLowerCase();
          req.params.hook = req.params.hook.toLowerCase();
          metric.hgetall('/' + req.params.owner + "/" + req.params.hook + '/report' , function (err, report) {
            if (err) {
              return res.end(err.message);
            }
            // format date times so they are easier to read
            try  {
              report.lastStart = df(new Date(Number(report.lastStart)), "mm/dd/yyyy HH:MM:ss Z");
            } catch (err) {
            }
            try  {
              report.lastEnd = df(new Date(Number(report.lastEnd)), "mm/dd/yyyy HH:MM:ss Z");
            } catch (err) {
            }
            try  {
              report.lastTime = ms(Number(report.lastTime));
            } catch (err) {
            }
            res.json(report);
          });
  ...
  // update the workers table - only used for workers /gateway ( can probably be removed / moved )
  updateWorkersTable(function (err) {
...
//
function updateWorkersTable (cb) {
